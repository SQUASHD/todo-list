...
export default class UI {
  ...
  static loadTodos(projectName) {
    Storage.getTodoList()
      ...
      .getTodos()
      .forEach((todo) => UI.createTodo(todo.name, todo.dueDate))
  }
  ...
  static loadProjectContent(projectName) {
    ...
    if (projectName !== 'Due') {
      projectView.innerHTML += `
        <button class="add-task-button button-todo" id="button-add-task">
        </button>`
    }
  ...
  static initAddProjectBtns() {
    ...
    addProjectBtn.addEventListener('click', UI.openCreateProjectForm)
  ...
  static addProject() {
    ...
    else if (Storage.getTodoList().getProject(projectName)) {
  ...
  static deleteProject(projectName, deleteButton) {
    if (deleteButton.classList.contains('active')) UI.clearProjectView();
